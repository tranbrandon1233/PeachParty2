...
{
public:
	Actor(StudentWorld* sw, const int imageID, int startX, int startY, int dir = 0, int depth = 0, double size = 1.0, bool baddie = false)  //Constructor for Actor class
		: GraphObject(imageID, startX, startY, dir, depth, size), m_alive(true), sw(sw), direction(dir), impacted(false), isBaddie(baddie)  //peachHead(new Coords), yoshiHead(new Coords), peachTemp(peachHead), yoshiTemp(yoshiHead)  //Initialize parameters
	{}
...
public:
	...
	int getStars() const { return stars; }  //Change stars
	int getTicks() const { return ticks_to_move; }  //Change stars
